...
依赖：docker compose v2，删除`name: fan-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name fan-maven-repo

docker run -it --rm --name fan-maven \
    -v fan-maven-repo:/root/.m2 \
